...
const createNewHeadmaster = async (req, res) => {
  ...
  try {
    ...
    const sql2 =
      "INSERT INTO users (username, email, password, school_name, gender, user_img, role_id, active) VALUES($1, $2, $3 , $4 , $5, $6, $7, $8) RETURNING *";
    ...
    res
      .status(201)
      .json({ success: `New user (${result.rows[0].username}) created!` });
  } catch (err) {
...
const updateHeadmaster = async (req, res) => {
  ...
  const headmaster = await pool.query(
    "SELECT * FROM users WHERE user_id = $1 AND role_id = '3'",
    [id]
  );
  ...
  const hashedPwd = await bcrypt.hash(password, 10);
  const sql =
    "UPDATE users SET username = $1, email = $2, password = $3, school_name = $4, gender = $5, role_id = '3', user_img = $6 WHERE user_id = $7";
  const updatedHeadmaster = await pool.query(sql, [
    username,
    email,
    hashedPwd,
    school_name,
...
const deleteHeadmaster = async (req, res) => {
  ...
  res.json({ success: `Headmaster ID ${id} Deleted.` });
};
...
const getHeadmaster = async (req, res) => {
  ...
  const headmaster = await pool.query(
    "SELECT * FROM users WHERE user_id = $1 AND role_id = '3' AND active = '1'",
    [id]
  );
...
const deleteHeadmasters = async (req, res) => {
  const Headmasters = await pool.query(
    "SELECT * FROM users WHERE active = '0' AND role_id = '3'"
  );
...
const activeHeadmaster = async (req, res) => {
  const { id } = req.params;
  if (!id) return res.json({ message: "Headmaster ID required." });

  const headmaster = await pool.query(
    "SELECT * FROM users WHERE user_id = $1 AND role_id = '3' ORDER BY user_id DESC",
    [id]
  );
  if (headmaster.rows.length === 0) {
    return res.json({ message: `No headmaster matches ID ${id}.` });
  ...
  const deletedHeadmaster = await pool.query(sql, [id]);
  res.json({ success: `Headmaster ID ${id} Activited.` });
};
