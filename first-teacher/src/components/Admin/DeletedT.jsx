...
const DeletedT = ({teachersUpdated,setTeachersUpdated }) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5500/teachers/deleted", {
        headers: {
  ...
  }, [refreshData, teachersUpdated]);

  const handleActive = (id) => {
    axios
      .put(
        `http://localhost:5500/teachers/active/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      .then((response) => {
        if (response.data.message) {
          ...
          setTeachersUpdated(!teachersUpdated)
        }
      })
      .catch((error) => {
        console.log(error);
  ...
  }, [isLoading, firstUserIndex, users, lastUserNumber, refreshData, teachersUpdated]);

  return (
    <>
     <div
        className="table-responsive small  mb-3"
      >
      <table
      ...
      {limitedUsers.length === 0 && (
                <h5 className="text-center text-white">لا يوجد معلمين محذوفين</h5>
              )}
    </>
  );
};
