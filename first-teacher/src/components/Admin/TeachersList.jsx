...
const TeachersList = ({teachersUpdated,setTeachersUpdated }) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5500/teachers", {
        headers: {
  ...
  const handleDelete = (id) => {
    axios
      .put(
        `http://localhost:5500/teachers/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  return (
   ...
   {limitedUsers.length === 0 && <h5 className="text-center text-white">لا يوجد معلمين</h5>}
    </>
  );
}
